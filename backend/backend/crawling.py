...
def getHtmlData():
    soup=BeautifulSoup(requests.get("http://www.bokjiro.go.kr/welInfo/retrieveGvmtWelInfo.do?welInfSno=310").text, features="html.parser")
    # print(soup)
    form = soup.find('form', attrs={'id':'frm'})
    inputTag = form.find('input', id="welInfDtlCn").get('value')
    
    print(inputTag)
    soup2 = BeautifulSoup(inputTag, features="html.parser")
    # print(soup2)
    divTag = soup2.find('ul', class_="bokjiBlit01")
    # print(divTag)
...
#     parser = BeautifulSoup(resp.content, 'html.parser')
    # formTag = parser.find('form', id='frm')
    # inputTag = formTag.find('input', id="welInfDtlCn").get('value')
    # print(inputTag)
    # print(parser)
    # lis = parser.find('input', {'id': 'welInfDtlCn'})
    # lis = HTMLParser().unescape(str(lis))
    # print(lis)
    # lis = BeautifulSoup(lis, 'html.parser')
    # divTags = lis.find_all('div')
